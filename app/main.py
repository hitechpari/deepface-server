...
async def search_face_base64(data: dict):
    """Lightweight search - single person at a time"""
    try:
        image_base64 = data.get('image')
        ...
        # Compare ONE person at a time to save memory
        results = []
        
        for person in persons:
            try:
                # Check if image_url exists and is valid (not example.com)
                if 'image_url' not in person or 'example.com' in person['image_url']:
                    logger.warning(f"⚠️ Skipping {person.get('name')}: invalid image_url")
                    continue
...
async def list_faces():
    ...
    for doc in docs:
        data = doc.to_dict()
        # Only include entries with valid image_url
        if data.get('image_url') and 'example.com' not in data['image_url']:
            faces.append({
                "id": doc.id,
                "name": data.get('name'),
                "age": data.get('age'),
